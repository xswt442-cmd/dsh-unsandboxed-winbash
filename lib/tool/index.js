...
import { resolveGitBashPath, runBash, startBash, terminateLiveChildren } from "./exec.js";
...
//#endregion
// dsh takes a plugin's metadata from the value it uses — the default export — so
// `inject` has to travel with the function, not only beside it. As a named export
// alone it leaves the loader with an undeclared `ctx.systemPrompt`, and that is a
// boot failure rather than a degraded tool: "plugin tree failed to load: … cannot
// get property \"systemPrompt\" without inject". dsh-treekeeper and dsh-ballast
// attach it this way, and their bundle rows declare no inject either.
apply.inject = inject;
