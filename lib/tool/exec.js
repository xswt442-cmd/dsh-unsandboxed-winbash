...
export function buildEnvironment(request, config, deps = {}) {
	...
	// `deps.resolveBashPath` keeps the environment builder testable off Windows: the
	// default resolver insists on a real Git for Windows install, which the unit job
	// on Linux does not have, while the PATH composition it feeds is platform-neutral.
